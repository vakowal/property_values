...
print(p)
pngname <- "D:/PODER_Emma/Results/Property_value_summary_by_segment.png"
png(filename=pngname, width=6, height=6, units='in', res=300)
print(p)
dev.off()

# why are maximum property values in lower 33% going up?
# maximum parcel value goes up by a lot
# the PIN for the property with maximum value in 2009 out of all lower-third parcels
# is 963819239800000
max_2009_val_pin <- 963819239800000
test_subs <- values_df[values_df$PIN == max_2009_val_pin, ]

# why are property values in middle and upper 33% going down?
# the minimum parcel value goes down by a lot
...
parcels_Emma <- merge(
  parcels_Emma_df, class_codes, by.x='Class', by.y='CODE', all.x=TRUE)
